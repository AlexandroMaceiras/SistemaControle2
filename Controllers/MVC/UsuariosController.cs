    ...
    public class UsuariosController : Controller
    {
        ...
        public ActionResult Create(UsuarioView view)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    //Salva pra criar o UserID que será usado na foto
                    db.SaveChanges();

                    //Sobe a foto com o UserId criado no nome
                    if (view.Foto != null)
                    {
                        var pic = Utilidades.UploadPhoto(view.Foto, view.Usuario.UserId);
                        if (!string.IsNullOrEmpty(pic)){
                            view.Usuario.Photo = string.Format("~/Content/Fotos/{0}", pic);
                        }
                    }
                    //Salva novamente para atualizar o registro no BD do nome da foto com o Id que foi salvo.
                    db.SaveChanges();
