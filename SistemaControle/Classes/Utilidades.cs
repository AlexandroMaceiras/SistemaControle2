...
namespace SistemaControle.Classes
{
    public class Utilidades
    {
        ...
        public static string UploadPhoto(HttpPostedFileBase file, int userId)
        {
            // Upload image
            ...
            if (file != null)
            {
                ...
                nomeFoto = userId.ToString() + Path.GetExtension(pic);
                path = Path.Combine(HttpContext.Current.Server.MapPath("~/Content/Fotos/"), userId.ToString()+ Path.GetExtension(pic));
                file.SaveAs(path);
